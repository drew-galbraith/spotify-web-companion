...
export function useAutoPlaylist() {
  ...
  const { spotifyToken: token, user } = useSafeAuth(); ;
  ...
  const createAutoPlaylist = async (tripId: string, destination: string, description: string, tripName?: string) => {
    ...
    try {
      ...
      // Save to local store immediately - only pass the ID
      addPlaylistToTrip(tripId, playlist.id);
      console.log(`Added playlist to trip: ${tripId}`, playlist.id);
      
      // Save to Firebase
      try {
        setCurrentStep("Saving playlist to database...");
        ...
        const playlistData = {
          ...
          countryCode: getCountryCode(destination.split(',').pop() || ''),
          userId: user.id,
          tripId: tripId,
        ...
        console.log("=== Saving Auto Playlist to Firebase ===");
        ...
        // Try to find trip in either root collection or user's subcollection
        let tripRef = doc(db, 'trips', tripId);
        let tripSnapshot = await getDoc(tripRef);
        
        if (!tripSnapshot.exists()) {
          tripRef = doc(db, 'users', user.id, 'trips', tripId);
          tripSnapshot = await getDoc(tripRef);
        }
        ...
        // Don't throw the error - continue with success since Spotify playlist was created
        console.warn("Playlist created on Spotify but failed to save to database");
      }
